...
async def test_update_user_to_duplicate_email_raises_error(
    user_service: UserService, existing_user: User
):
    """
    ...
    second_user_in = UserCreate(
        full_name="Second User", email="second@example.com", password="password123"
    )
    ...
    # 执行并断言 (Act & Assert)
    with pytest.raises(
        ValueError, match="This email is already registered to another user."
    ):
...
async def test_authenticate_user_success(
    ...
    authenticated_user = await user_service.authenticate_user(
        existing_user.email, plain_password
    )
...
async def test_authenticate_user_nonexistent_email_returns_none(
